...
class User::AsFacebookSignUp < ActiveType::Record[User]
  validates_presence_of :auth_provider
  validates_presence_of :auth_uid
  validates_inclusion_of :auth_provider, in: ['facebook']

  before_validation :set_username, on: :create
  before_validation :set_random_password, on: :create

  default_scope { where(auth_provider: 'facebook') }
  ...
  def set_username
    self.username = "fb:#{self.auth_uid}"
  end
